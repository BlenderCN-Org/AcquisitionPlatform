...
void CALLPASCAL CALLBACK SrcEventProc( NKREF refProc, ULONG ulEvent, NKPARAM data )
{
	...
	switch(ulEvent){
		...
		case kNkMAIDEvent_RecordingInterrupted:
			printf( "Recording was stopped by error(%d).\n", data );
			break;
		default:
			printf( "Detected unknown Event to the Source object.\n" );
		}
}
//------------------------------------------------------------------------------------------------------------------------------------
...
NKERROR CALLPASCAL CALLBACK DataProc( NKREF ref, LPVOID pInfo, LPVOID pData )
{
	...
	if ( pDataInfo->ulType & kNkMAIDDataObjType_File ) {
		...
		if( ulOffset < pFileInfo->ulTotalLength ) {
			...
			//When the delivery has finished, save the pointer to the data and the interesting variables, such as dimension or file type. 
			switch( pFileInfo->ulFileDataType ) {
				case kNkMAIDFileDataType_JPEG:
					//jpeg
					bufferImage.wPhysicalBytes= 1;
					bufferImage.JpegOrNef = 0;
					break;
				case kNkMAIDFileDataType_TIFF:
					//raw
					bufferImage.wPhysicalBytes= 2;
					bufferImage.JpegOrNef = 1;
					break;
				case kNkMAIDFileDataType_NIF:
					...
					break;
				default:
					bufferImage.JpegOrNef = 2;
			}

			bufferImage.ulElements = pFileInfo->ulTotalLength;
			bufferImage.pData = ((LPRefDataProc)ref)->pBuffer;
