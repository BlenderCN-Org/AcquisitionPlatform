...
plot([x(1) x(end)], [y(1) y(end)],'k','LineWidth',1)
...
Llums(handles.data,[pos(1,1) pos(1,2)]);
colors=[colors;[pos(1,1) pos(1,2)]]; 
end
...
%     end
    c=rgb <= 0.0031308;
    rgb(c) = 12.92*rgb(c);
    rgb(~c) = 1.055*(rgb(~c).^(1.0/2.4)) - 0.055;
    rgb=rgb*1.4;
    rgb(rgb>1)=1;
    rgb(rgb<0)=0;
